...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
    ...
    conn.on('qr', async qr => {
      let scan = await parent.sendFile(m.chat, await qrcode.toDataURL(qr, { scale: 8 }), 'qrcode.png', '*QR Code Akan Expired Dalam 20 Detik!*', m)
      setTimeout(() => {
        parent.deleteMessage(m.chat, scan.key)
      }, 30000)
    })
    conn.version =[2,2143,3]
    conn.browserDescription =['Whangsaff Bot', 'Edge', '3.0']
      ...
      setTimeout(async () => {
        try {
          ...
          if (conn.user && conn.user.jid)
            parent.sendMessage(conn.user.jid, `[ *!* ] Error.. Message: Sambungan Terputus..`, MessageType.extendedText)
          let i = global.conns.indexOf(conn)
          if (i < 0) return
          delete global.conns[i]
          global.conns.splice(i, 1)
